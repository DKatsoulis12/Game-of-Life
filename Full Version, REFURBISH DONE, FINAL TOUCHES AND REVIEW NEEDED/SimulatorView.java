...
//originally
    ...
    public void start(Stage stage) {
        ...
        int simulateGenerationNum = 100;
        simulate(simulateGenerationNum-1);
    }
        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                ...
                if (cell != null && cell.isAlive()) {
                    ...
                    fieldCanvas.drawMark(col, row, cell.getAliveColor());
                }
                else {
                    fieldCanvas.drawMark(col, row, cell.getDeadColor());
                }
            }
        }
    ...
    new Thread(() -> {
        for (int gen = 0; gen <= numGenerations; gen++) {
            ...
            simulator.delay(4000);  
            Platform.runLater(() -> {
                updateCanvas(simulator.getGeneration(), simulator.getField());
            });
            try {
                Thread.sleep(1000);  // Introduce a slight delay for synchronization - CHANGE DELAY HERE AS NEEDED
   ...
   public static FieldStats getFieldStats(){
       return stats;
    }
}
